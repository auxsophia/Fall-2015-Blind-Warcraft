...
function JocysCom_OptionsFrame_OnEvent(self, event, arg1, arg2)
	...
	elseif JocysCom_DialogueMiniFrame:IsShown() and (string.find(event, "QUEST") ~= nil or event == "GOSSIP_SHOW" or event == "ITEM_TEXT_READY") then
		...
		elseif event == "QUEST_DETAIL" then
			...
			speakMessage = GetQuestText() .. " " .. objectivesHeader .. " " .. GetObjectiveText().."Zaboomafoo"; --DEBUG
	...
	if string.find(event, "WHISPER") ~= nil then messageType = " whispers. "end --DEBUG
	if string.find(event, "YELL") ~= nil then messageType = " yells. " end

	-- Final message.
	if (NameIntro == true) then
		--speakMessage = messageLeader .. arg2 .. " " .. messageType .. speakMessage.."yo waddup"; --DEBUG
	...
	--so far, if the quest log is open, you can whisper whatever to yourself
	--and Jocys will read the paragraph about the quest (instead of the play button)
	--message(speakMessage.."::before IF");	--DEBUG
	--speakMessage = "Mazgrind whispers. readAgain";
	--if (speakMessage == "Mazgrind whispers. readAgain") then
		--message(speakMessage.."yup");	--DEBUG
...
function JocysCom_SpeakMessage(speakMessage, event, name, group)
	...
	message(speakMessage.."speakMessage func");	--DEBUG
...
-- Show MiniMenuFrame and set text.
 function JocysCom_MiniMenuFrame_Show(name)
	...
	elseif name == "SoundBattlegroundLeader" then
		fontString = "|cffff4709Play sound at the beginning of BATTLEGROUND-LDR msg|r";
	...
	elseif name == "NameBattlegroundLeader" then
		fontString = "|cffff4709Add \"<Name> says.\" to BATTLEGROUND-L messages|r";
...
function JocysCom_LoadTocFileSettings()
	...
	if JocysCom_StopOnCloseCB == true then JocysCom_StopOnCloseCheckButton:SetChecked(true) else JocysCom_StopOnCloseCheckButton:SetChecked(false) end
